...
function BookmarksPage() {
  ...
  return (
    <Box sx = {{
      ...
      <Backdrop
        sx={{ color: '#fff', zIndex: (theme) => theme.zIndex.drawer + 1, opacity: 2}}
        open={open}
        onClick={handleClose}
      ...
      <Grid container spacing={2} direction="row" sx = {{ mt : 5}}>
        {bookmarks.map((bookmark, index) => (
          <Slide direction="up" in={true} timeout = { (index * 500) + 1000} mountOnEnter key={index}>
            <Grid item xs={4}>
              <Card >
                <CardContent>
                  <Typography
                ...
                <CardActions >
                  <Button onClick = {() => handleToggle(index)} size="small">View Job</Button>
                </CardActions>
              </Card>
            </Grid>
          </Slide>
